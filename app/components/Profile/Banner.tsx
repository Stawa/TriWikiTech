...
function ProfileBanner({
  ...
  return (
    <div className="w-full h-full">
      <div className="bg-white dark:bg-indigo-900 bg-opacity-20 dark:bg-opacity-20 shadow-lg rounded-lg p-4 lg:p-6 transition-all duration-300 hover:shadow-2xl relative overflow-hidden h-full">
      ...
      <div className="flex flex-wrap justify-center gap-1 lg:gap-2 mt-1 lg:mt-2">
        {user.displayedBadges &&
          user.displayedBadges.map((badge: string, index: number) => (
            <span
              key={index}
              className="px-2 lg:px-3 py-1 text-xs font-semibold text-white bg-indigo-500 dark:bg-indigo-600 rounded-full flex items-center shadow-md"
            >
              {badges[badge]}
...
function ProfileBio({ bio }: { bio: string }) {
  return (
    <div className="bg-gray-100 dark:bg-indigo-800 bg-opacity-30 dark:bg-opacity-30 rounded-xl p-3 lg:p-4 mb-4 lg:mb-6 shadow-inner">
      <h3 className="text-lg lg:text-xl font-semibold text-indigo-600 dark:text-indigo-300 mb-1 lg:mb-2 flex items-center">
        <FaUserCircle className="mr-2" /> Bio
      </h3>
      <p className="text-gray-700 dark:text-indigo-200 text-xs lg:text-sm leading-relaxed">
...
function ProfileActions({ handleOpenModal }: { handleOpenModal: () => void }) {
  return (
    <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2 lg:space-x-4">
      <button className="flex-1 bg-purple-600 hover:bg-purple-700 dark:bg-purple-700 dark:hover:bg-purple-800 text-white font-bold py-1 lg:py-2 px-2 lg:px-4 rounded-md transition duration-300 flex items-center justify-center text-xs lg:text-sm">
        <FaEdit className="mr-1 lg:mr-2" /> Edit Profile
      </button>
      <button
        onClick={handleOpenModal}
        className="flex-1 border-2 border-indigo-500 dark:border-indigo-400 text-indigo-500 dark:text-indigo-400 hover:bg-indigo-500 dark:hover:bg-indigo-400 hover:text-white dark:hover:text-black font-bold py-1 lg:py-2 px-2 lg:px-4 rounded-md transition duration-300 flex items-center justify-center text-xs lg:text-sm"
      >
